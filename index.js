const { Client, MessageAttachment, Collection, Intents, MessageEmbed  } = require('discord.js');
// const db = require('quick.db')
const Discord = require('discord.js');
const {token , PREFIX, DBL_API_KEY } = require('./config');
...
const dbl = new DBL(process.env.DBL_API_KEY)
const fs = require("fs");
const db = require('quick.db');
const jimp = require('jimp');

const uptime = require("discord-uptime-bot");
uptime.webserver(6969)
...
// bot.categories = fs.readdirSync("./SlashCommands/");
// const rest = new REST({ version: '10' }).setToken('token');
["command"].forEach(handler => {
    require(`./handler/${handler}`)(bot);
});

fs.readdir("./events/guild", (_err, files) => {
  files.forEach(file => {
    ...
    delete require.cache[require.resolve(`./events/guild/${file}`)];
  });
});
fs.readdir("./events/client", (_err, files) => {
  files.forEach(file => {
    if (!file.endsWith(".js")) return;
...
//             let menprefix = new MessageEmbed()
//           .setAuthor(`${message.guild.me.displayName} `, bot.user.displayAvatarURL())
//           .setThumbnail(message.guild.iconURL())
//           .setColor('#f5ce42')
//           .setDescription(`${message.author} **My Prefix In This Server is - \`${prefix}\`**`)    .setImage('https://media.discordapp.net/attachments/999522063683440692/1001851329297264720/standard.gif')
//           .setFooter(`All right reserved by [ Coding ] 👑 Venom#6969` ,  bot.user.displayAvatarURL())
//           .setTimestamp()
//           message.channel.send(menprefix).then(msg => {
...
require('./keepAlive')();
bot.login(process.env.TOKEN)&& uptime.client(process.env.TOKEN, PREFIX);
