...
module.exports = {
  ...
  category: "owner",
    ...
    run: async(bot, message, args) => {
        ...
        //This will check weather there is data in thanks leaderboard otherwise the bot will send this
        if(!thanks.length) {
        ...
        for (var i in thanks) {
            ...
            finalLb += `**${thanks.indexOf(thanks[i]) + 1}. ${bot.users.cache.get(thanks[i].ID.split('_')[1]) ? bot.users.cache.get(thanks[i].ID.split('_')[1]).tag : "Unknown User#0000"}** - \`${thanks[i].data} Thanks\`\n`;
        };
